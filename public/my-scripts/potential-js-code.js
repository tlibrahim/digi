...
function editProfiling(event, profile_id) {
    ...
    var formData = new FormData(),
        formDataUrl = '{{ url("potentials/upload-logo") }}/' + profile_id + '/' + url.split("/")[url.split("/").length - 1]
    var logo = form.find('input[type="file"]:first')[0].files[0]
    if (logo != undefined) {
        if (logo.size > 1024 * 4 * 1000) {
            swal("Allowed Size 4MB", {
                icon: 'error'
            })
            return
        }
        if (logo.type != 'image/png' && logo.type != 'image/jpg' && logo.type != 'image/jpeg') {
            ...
            swal("Allowed Extensions [png ,jpg ,jpeg]", {
                icon: 'error'
            })
            return
        }
    ...
    $.ajax({
        ...
        success: function(rData) {
            if (rData.status == 'ok') {
                ...
                if (progress >= 46) {
                    var tr = $('#tab01').find("#potentials-" + rData.id).html()
                    $('#tab01').find("#potentials-" + rData.id).remove()
                    $("#tab02").find('#potentials-render-table-2 #potentials-table-2').append(tr)
                    dTables = $('table').DataTable()
                } else if (progress >= 86) {
                    var tr = $('#tab01').find("#potentials-" + rData.id).html()
                    $('#tab02').find("#potentials-" + rData.id).remove()
                    $("#tab03").find('#potentials3render-table-3 #potentials-table-2').append(tr)
                    dTables = $('table').DataTable()
        ...
        error: function(erData) {
            swal('Can`t update this profiling', {
                icon: 'error'
            })
        }
    })
                ...
                $.ajax({
                    ...
                    success: function(rData) {
                        ...
                        if (progress >= 46) {
                            ...
                            var tr = $('#tab01').find("#potentials-" + rData.id).html()
                            $('#tab02').find("#potentials-" + rData.id).remove()
                            $("#tab03").find('#potentials-r3nder-table-3 #potentials-table-2').append(tr)
                            dTables = $('table').DataTable()
        ...
        success: function(rData) {
            if (rData.status == 'ok') {
                ...
                if (progress >= 46) {
                    ...
                    var tr = $('#tab01').find("#potentials-" + rData.id).html()
                    $('#tab02').find("#potentials-" + rData.id).remove()
                    $("#tab03").find('#potentials-render-3able-2 3potentials-table-2').append(tr)
                    dTables = $('table').DataTable()
            ...
            swal(rData.msg, {
                ...
                if (rData.msg != 'error') {
                    ...
                    if (progress >= 46) {
                        ...
                        var tr = $('#tab01').find("#potentials-" + rData.id).html()
                        $('#tab02').find("#potentials-" + rData.id).remove()
                        $("#tab03").find('#potentials-render-table-3 #potentials-table-3').append(tr)
                        dTables = $('table').DataTable()
